...
module.exports = {
  ...
  rules: {
    //表示这个no-console选项在生产阶段是有问题，在开发阶段关闭
    "no-console": process.env.NODE_ENV === "production" ? "error" : "off",
    //不允许出现debugger语句，在生产阶段不许出现debugger
    "no-debugger": process.env.NODE_ENV === "production" ? "error" : "off",
    "prettier/prettier": "off",
    // 在用Prettier格式化的时候，可以能会和ESLint定义的校验规则冲突，
    // 比如Prettier字符串默认是用双引号而ESLint定义的是单引号的话这样格式化之后就不符合ESLint规则了。
    // 所以要解决冲突就需要在Prettier的规则配置里也配置上和ESLint一样的规则，这里贴下ESLint和Prettier的配置文件。
    // 不强制使用单引号，启用后报很多错
    quotes: ["off", "single"],
    //不检查分号结尾
    semi: ["off", "never"],
    ...
    //[vue/no-parsing-error] Parsing error: x-invalid-end-tag。 在.eslintrc.js中添加的规则 在vscode的settings 里面的配置如下：
    ...
    "no-multiple-empty-lines": [2, { max: 5 }], //空行最多不能超过5行
